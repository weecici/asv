...
# ======================================================================#
_nb_proc = 16  # numer of sub-processes (set 1 for single process)
...
_dir_dataset = "./data/"  # directory of Dataset
    ...
    for r, ds, fs in os.walk(_dir_dataset):
        for f in fs:
            if os.path.splitext(f)[1] != ".wav":
                continue
            l_utt.append("/".join([r, f]))

    print(f"Found {len(l_utt)} audio files")
    if len(l_utt) == 0:
        exit(1)
